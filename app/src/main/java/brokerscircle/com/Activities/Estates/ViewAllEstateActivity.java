...
import brokerscircle.com.adapters.RealEstateRecyclerview_Config;
...
public class ViewAllEstateActivity extends AppCompatActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener, View.OnClickListener, PullRefreshLayout.OnRefreshListener {
    ...
    private static final String TAG = "ViewAllEstateActivity";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (user == null) {    //Check if user if logged in
            ...
            Intent intent = new Intent(ViewAllEstateActivity.this, LoginActivity.class);
    ...
    private void setEstateBottomSheetData(RealEstateData realEstateData) {
        ...
        }, ViewAllEstateActivity.this,user.getResponse().getData().getUser().getUserId(),realEstateData.getId(), "Company");
...
//            }
        ...
        new FavoriteDatabaseHelper().readFavoriteWithCreatedUserIdAndReferenceID_AND_Type(new FavoriteDatabaseHelper.DataStatus() {
            ...
            public void DataIsLoaded(List<FavoriteData> favoriteData) {
                if (!favoriteData.isEmpty()){
                    ...
                    },ViewAllEstateActivity.this, params);
                }
            }
            ...
            public void DataIsDeleted(List<FavoritePostResponseUtils> responseUtils) {
                if (responseUtils.get(0).getError().equals("false")){
                    Toast.makeText(ViewAllEstateActivity.this, "Successfully Removed", Toast.LENGTH_SHORT).show();
                    mFavoriteBTN.setChecked(false);
                }else {
                    Toast.makeText(ViewAllEstateActivity.this, responseUtils.get(0).getMessage(), Toast.LENGTH_SHORT).show();
                    mFavoriteBTN.setChecked(true);
                }
            }
        }, ViewAllEstateActivity.this,user.getResponse().getData().getUser().getUserId(),realEstateData.getId(), "Company");
    ...
    private void FavoriteMethod() {
        ...
        new FavoriteDatabaseHelper().postFavorite(new FavoriteDatabaseHelper.DataStatus() {
            ...
            public void DataIsPosted(List<FavoritePostResponseUtils> responseUtils) {
                if (responseUtils.get(0).getError().equals("false")){
                    Toast.makeText(ViewAllEstateActivity.this, "Successfully Saved", Toast.LENGTH_SHORT).show();
                    mFavoriteBTN.setChecked(true);
                }else {
                    Toast.makeText(ViewAllEstateActivity.this, responseUtils.get(0).getMessage(), Toast.LENGTH_SHORT).show();
                    mFavoriteBTN.setChecked(false);
